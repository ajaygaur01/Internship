    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-6">
            {post.map((c, index) => (
                ...
                >
                    <img
                        src={`http://localhost:8000/images/${c.photo}`}
                        className="w-full h-48 object-cover rounded-t-xl"
                        alt={c.topic}
                    />
                    <div className="p-6">
                        <h2 className="text-2xl font-semibold mb-2 text-gray-800">{c.topic}</h2>
                        <p className="text-gray-500 italic">{c.description}</p>
                    </div>
                    <div className="p-4 bg-gray-100 border-t border-gray-200 flex justify-between items-center rounded-b-xl">
                        {/* Use the Like component */}
                        <Like />
